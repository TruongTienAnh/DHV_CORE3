    ...
    echo '
        <div class="toast-container position-fixed bottom-0 end-0 p-3">
            <div class="toast align-items-center text-white bg-' . htmlspecialchars($type) . ' border-0' . $class . '"' . $id . $attr . ' role="alert" aria-live="assertive" aria-atomic="true">
                <div class="d-flex align-items-center justify-content-between" style="padding-right: var(--bs-toast-padding-x);">
...
// Select In Modal Component
$app->setComponent('select-modal', function ($vars) {
    $name = isset($vars['name']) ? $vars['name'] : '';
    $options = isset($vars['options']) ? $vars['options'] : [];
    $class = isset($vars['class']) ? ' ' . htmlspecialchars($vars['class']) : '';
    $id = isset($vars['id']) ? ' id="' . htmlspecialchars($vars['id']) . '"' : '';
    $attr = isset($vars['attr']) ? $vars['attr'] : '';
    $selections = isset($vars['selections']) ? $vars['selections'] : [];

    echo '
        <select data-select data-style="form-select bg-body-tertiary py-3 rounded-4 w-100" 
            data-live-search="true" class="' . $class . '"' . $id . ' 
            name="' . htmlspecialchars($name) . '[]" ' . $attr . ' multiple>';
    foreach ($options as $option) {
        $isSelected = in_array($option['value'], $selections) ? ' selected' : '';
        echo '<option value="' . htmlspecialchars($option['value']) . '"' . $isSelected . '>' . htmlspecialchars($option['text']) . '</option>';
    }
    echo '</select>';
});
...
$app->setComponent('breadcrumb', function ($vars) use ($app, $jatbi) {
    include('templates/components/breadcrumb.php');
});

$app->setComponent('header_frontend', function ($vars) use ($app,$jatbi) {
    require_once('templates/frontend/header.html');
});
$app->setComponent('footer_frontend', function ($vars) use ($app,$jatbi) {
    require_once('templates/frontend/footer.html');
});