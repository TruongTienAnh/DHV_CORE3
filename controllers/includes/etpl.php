...
class etpl
{
    ...
    public function set(string $selectorWithCommand, $value): void
    {
        ...
        if (strpos($command, self::APPEND . ':') === 0 || strpos($command, self::PREPEND . ':') === 0) {
    ...
    private function applyElementData(DOMDocument $dom, DOMElement $element, array $data): void
    {
        ...
        if (isset($data[self::TEXT_CONTENT])) {
        ...
        foreach ($data as $key => $value) {
            if (strpos($key, '|') !== false) {
                ...
                foreach ($nodes as $node) {
                    ...
                    if (strpos($modifier, self::STYLE . ':') === 0) {
                        ...
                        // Thêm hoặc sửa thuộc tính style
