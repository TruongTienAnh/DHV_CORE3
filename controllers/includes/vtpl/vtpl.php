...
class Vtpl
{
	...
	private function processVariables()
	{
		...
		usort($variables[1], function ($a, $b) {
		...
		if (isset($this->selectors) && isset($this->document) && isset($this->xpath)) {
			...
			foreach ($this->selectors as &$data) {
				...
				$selector = preg_replace_callback(
					'/^@([a-zA-Z][a-zA-Z0-9_-]+)(?![a-zA-Z0-9_-])\s+=\s+(.+)$/',
				...
				if (isset($this->variableFilters[$name])) {
					...
					if ($type == 'class') {
						...
						if ($type == 'tag') {
							...
							$closeTag = preg_replace_callback(
								'@\$\$(\d+)@',
								function ($matches) use ($options) {
									if ($matches[1] > 0) {
										$options[$matches[1]] = '\'' . trim($options[$matches[1]], '\'') . '\'';
	...
	private function cleanHtml(&$html)
	{
		...
		$html = preg_replace_callback(
			'/@@__VTPL__ATTRIBUTE_PLACEHOLDER__(\d+)@@/',
		...
		$html = preg_replace_callback(
			'/__VTPL__NEW_ATTRIBUTE_PLACEHOLDER__(\d+)=""/',
			function ($matches) use ($self) {
		...
		$html = preg_replace_callback(
			'/<script(.*?)holder="@@__VTPL__SCRIPT_PLACEHOLDER__(\d+)@@"(.*?)><\/script>/',
			function ($matches) use ($self) {
				if (VTPL_JS_MINIFY) {
