...
$libraryHandler = function($vars) use ($app, $jatbi, $view, $setting) {
    $slug = $vars['slug'] ?? '';
...
// Đăng ký 2 route dùng chung handler
...
$app->router("/library-detail/library-add/{slug}", 'GET', function($vars) use ($app, $jatbi, $setting) {
    ...
    echo $app->render('templates/frontend/library-post.html', $vars, 'global');
});
...
$libraryDetailHandler = function($vars) use ($app, $jatbi, $view, $setting) {
    $slug = $vars['slug'] ?? null;
    ...
    $view->setting = $setting; // Truyền setting để dùng template path

    $view->render('library-detail.html');
};

// Đăng ký route
$app->router("/library-detail/{slug}", 'GET', $libraryDetailHandler);
