...
$common = $app->getValueData('common');
    ...
    $app->router("/admin/blockip", 'POST', function ($vars) use ($app, $jatbi, $setting) {
        $app->header([
        ...
        $app->select("blockip", "*", $where, function ($data) use (&$datas, $jatbi, $app, $setting) {
            $datas[] = [
                ...
                "action" => $app->component("action", [
                    "button" => [
                        [
                            ...
                            'action' => ['data-url' => $setting['backend'] . '/admin/blockip-edit/' . $data['active'], 'data-action' => 'modal']
                        ],
                        [
                            'type' => 'button',
                            'name' => $jatbi->lang("Xóa"),
                            'permission' => ['blockip.deleted'],
                            'action' => ['data-url' => $setting['backend'] . '/admin/blockip-deleted?box=' . $data['active'], 'data-action' => 'modal']
                        ],
                    ]
                ]),
            ];
    ...
    $app->router("/admin/logs", 'POST', function ($vars) use ($app, $jatbi, $setting) {
        $app->header([
            'Content-Type' => 'application/json',
        ]);
        $draw = isset($_POST['draw']) ? intval($_POST['draw']) : 0;
    ...
    $app->router("/admin/trash", 'POST', function ($vars) use ($app, $jatbi, $setting) {
        $app->header([
            'Content-Type' => 'application/json',
        ]);
        $draw = isset($_POST['draw']) ? intval($_POST['draw']) : 0;
        ...
        $app->select(
            ...
            function ($data) use (&$datas, $jatbi, $app, $setting) {
                $datas[] = [
                    "checkbox" => $app->component("box", ["data" => $data['active']]),
                    "user" => '<img src="' . getAvatar($data['avatar']) . '" class="width rounded-circle me-2" style="--width:40px;aspect-ratio: 1/1;"> ' . $data['name'],
                    ...
                    "action" => $app->component("action", [
                        "button" => [
                            [
                                ...
                                'permission' => ['trash'],
                                'action' => ['data-url' => $setting['backend'] . $data['router'] . '/' . $data['active'], 'data-action' => 'modal']
                            ],
                            [
                                'type' => 'button',
                                'name' => $jatbi->lang("Xóa vĩnh viễn"),
                                'permission' => ['trash'],
                                'action' => ['data-url' => $setting['backend'] . '/admin/trash-deleted?box=' . $data['active'], 'data-action' => 'modal']
                            ],
                        ]
                    ]),
                ];
