...
$app->group($setting['backend'], function ($app) use ($jatbi, $setting, $permission) {
    ...
    $app->router("/users/logs", 'POST', function ($vars) use ($app, $jatbi, $setting) {
        $app->header([
            'Content-Type' => 'application/json',
        ]);
        $draw = isset($_POST['draw']) ? intval($_POST['draw']) : 0;
        ...
        $app->select(
            ...
            $where,
            function ($data) use (&$datas, $jatbi, $setting) {
                $datas[] = [
                    "user" => '<img src="' . getAvatar($data['avatar']) . '" class="width rounded-circle me-2" style="--width:40px;aspect-ratio: 1/1;object-fit: cover;"> ' . $data['name'],
                    "dispatch" => $data['dispatch'],
                    "action" => $data['action'],
                    "url" => $data['url'],
                    "ip" => $data['ip'],
                    "date" => $data['date'],
                    "views" => '<button data-action="modal" data-url='. $setting['backend'] . '"/admin/logs-views/' . $data['active'] . '" class="btn btn-primary-light btn-sm border-0 py-1 px-2 rounded-3" aria-label="' . $jatbi->lang('Xem') . '"><i class="ti ti-eye"></i></button>',
                ];
            }
        );
        echo json_encode([
    ...
    $app->router("/users/accounts", 'POST', function ($vars) use ($app, $jatbi, $setting) {
        $app->header([
            'Content-Type' => 'application/json',
        ]);
        $draw = isset($_POST['draw']) ? intval($_POST['draw']) : 0;
        ...
        $app->select(
            ...
            $where,
            function ($data) use (&$datas, $jatbi, $app, $setting) {
                $datas[] = [
                    "checkbox" => $app->component("box", ["data" => $data['active']]),
                    "name" => '<img src="' . getAvatar($data['avatar']) . '" class="width rounded-circle me-2" style="--width:40px; aspect-ratio: 1/1;object-fit: cover;"> ' . $data['name'],
                    "email" => $data['email'],
                    ...
                    "action" => $app->component("action", [
                        "button" => [
                            [
                                ...
                                'action' => ['data-url' => $setting['backend'] . '/users/accounts-edit/' . $data['active'], 'data-action' => 'modal']
                            ],
                            [
                                'type' => 'button',
                                'name' => $jatbi->lang("Xóa"),
                                'permission' => ['accounts.deleted'],
                                'action' => ['data-url' => $setting['backend'] . '/users/accounts-deleted?box=' . $data['active'], 'data-action' => 'modal']
                            ],
                        ]
                    ]),
                ];
    ...
    $app->router("/users/accounts-add", 'POST', function ($vars) use ($app, $jatbi, $setting) {
        $app->header([
            'Content-Type' => 'application/json',
        ]);
        ...
        if (empty($error)) {
            ...
            $directory = "datas/imgs";
            $insert['avatar'] = uploadFile($app, $jatbi, $_FILES['avatar'], $directory, $setting);
    ...
    $app->router("/users/accounts-edit/{id}", 'POST', function ($vars) use ($app, $jatbi, $setting) {
        $app->header([
            'Content-Type' => 'application/json',
        ]);
        ...
        if ($data > 1) {
            ...
            if (empty($error)) {
                $insert = [
                    ...
                    "gender"        => $app->xss($_POST['gender']),
                    "birthday"      => $app->xss($_POST['birthday']),
                    "password"      => ($_POST['password'] == '' ? $data['password'] : password_hash($app->xss($_POST['password']), PASSWORD_DEFAULT)),
                    "active"        => $data['active'],
                    "date"          => date('Y-m-d H:i:s'),
                    "status"        => $app->xss($_POST['status']),
                    "lang"          => $data['lang'] ?? 'vi',
                ...
                if ($_FILES['avatar']['size'] > 0) {
                    try {
                        $directory = "datas/imgs";
                        deleteFile($directory, $data['avatar']);
                        $insert['avatar'] = uploadFile($app, $jatbi, $_FILES['avatar'], $directory,$setting);
                    } catch (\Throwable $th) {
    ...
    $app->router("/users/permission", 'POST', function ($vars) use ($app, $jatbi, $setting) {
        $app->header([
            'Content-Type' => 'application/json',
        ]);
        $draw = isset($_POST['draw']) ? intval($_POST['draw']) : 0;
        ...
        $app->select("permissions", "*", $where, function ($data) use (&$datas, $jatbi, $app, $setting) {
            $datas[] = [
                "checkbox" => $app->component("box", ["data" => $data['active']]),
                "name" => $data['name'],
                ...
                "action" => $app->component("action", [
                    "button" => [
                        [
                            ...
                            'action' => ['data-url' => $setting['backend'] . '/users/permission-edit/' . $data['active'], 'data-action' => 'modal']
                        ],
                        [
                            'type' => 'button',
                            'name' => $jatbi->lang("Xóa"),
                            'permission' => ['permission.deleted'],
                            'action' => ['data-url' => $setting['backend'] . '/users/permission-deleted?box=' . $data['active'], 'data-action' => 'modal']
                        ],
                    ]
                ]),
            ];
