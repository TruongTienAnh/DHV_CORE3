  ...
  _initBox: function () {
    ...
    document
      ...
      .addEventListener("click", function (event) {
        ...
        if (
          top + addSectionBox.offsetHeight + self.frameHtml.scrollTop >
          outerHeight
        )
    ...
    addSectionBox.addEventListener("click", function (event) {
      ...
      if (element) {
        ...
        addSectionComponent(
          html,
          document.querySelector("[name='add-section-insert-mode']:checked")
            .value == "after"
        );
...
/* function displayToast(bg, title, message, id = "top-toast") {
  ...
  toast.querySelector(".toast-body .message").innerHTML = message.replace(
    /(?:\r\n|\r|\n)/g,
    "<br>"
  );

  // Hiển thị toast bằng JS
 ...
 */
function displayToast(type, title, message, id = "custom-toast-container") {
  ...
  toast.classList.add("custom-toast", `toast-${type}`, "showing");
  ...
  // Sự kiện đóng toast khi click vào nút đóng
  toast
...
function hideToast(toast) {
  ...
  setTimeout(() => toast.remove(), 500); // Xóa khỏi DOM sau hiệu ứng
}
  ...
  newPage: function () {
    ...
    let submitForm = function (e) {
      ...
      if (data["file"]) {
        ...
        data["url"] = data["file"] =
          "templates/frontend/" + data["folder"] + "/" + data["file"];
